...
- **Player "Fuente":** `Sintetizado` uses the Web Audio tropes; each rabbi appears as one **versión** after `GET /api/recordings` (includes `uploaderId`). Playback stitches the newest/shortest covering clip per verse, so a re-recorded fraction replaces that span. After a take, `POST /api/recordings/align` transcribes with Gemini (3.5 Transcribe when the gateway routes it, else `gemini-2.5-flash` + verse texts) and fills the verse range. `netlify dev` + `GET /api/dev-seed?parasha=Bereshit&aliyah=1` is enough to test two versions without Identity.
